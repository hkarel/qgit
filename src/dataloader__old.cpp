...
DataLoader::DataLoader(Git* g, FileHistory* f) : QProcess(g), git(g), fileHist(f) {
...
//    }
    ...
    if (len == -1) {
        emit loaded(fileHist, loadedBytes, loadTime.elapsed(), true, "", "");
        deleteLater();
        ...
        loadedBytes += len;
        if (timerCallCounter <= 5)
        {
            //qApp->processEvents();
            emit newDataReady(fileHist);
            //qApp->processEvents(QEventLoop::X11ExcludeTimers);
        }
    }
    ...
    if (procFinished) {
        ...
        //guiUpdateTimer.start(0);
        //qApp->processEvents();
        QMetaObject::invokeMethod(this, "on_timeout",  Qt::QueuedConnection);
    }
    else
