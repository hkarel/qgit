...
/*
const QString Git::refAsShortHash(const QString& sha) {
...
bool Git::getPatchFilter(const QString& exp, bool useGKey, bool isRegExp, ShaSet& shaSet) {
    ...
    QStringList sl = runOutput.split('\n', QGIT_SPLITBEHAVIOR(SkipEmptyParts));
    for (const QString& s : sl)
        shaSet.insert(s);
...
bool Git::getRefs() {
    ...
    QStringList rLst = runOutput.split('\n', QGIT_SPLITBEHAVIOR(SkipEmptyParts));
...
void Git::parseStGitPatches(const QStringList& patchNames, const QStringList& patchShas) {
    ...
    QStringList pl = runOutput.split('\n', QGIT_SPLITBEHAVIOR(SkipEmptyParts));
...
void Git::setExtStatus(RevFile& rf, const QString& rowSt, int parNum, FileNamesLoader& fl) {
    ...
    QStringList sl = rowSt.split('\t', QGIT_SPLITBEHAVIOR(SkipEmptyParts));
    if (sl.count() != 3) {
